...
const SELECT_VIDEO_ACTIONS = (s: VideoStore) => ({
	...
	setMuted: s.actions.player.setMuted,
...
export const usePlayer = () => {
	...
	const { setPlayerInitialized, setPlayerFileLoaded, setAudioTrack, setPlaying, setMuted, setVolume, setCursor } =
	...
	const propsSub = useCallback(({ name, data }: MpvPropertyEvent) => {
		switch (name) {
			...
			case "time-pos":
				...
				cursorRafRef.current = requestAnimationFrame(() => setCursor(pos));

				return;
			case "mute":
				if (typeof data !== "boolean") return;
				return setMuted(data);
			case "volume":
				if (typeof data !== "number") return;
	...
	useEffect(() => () => cancelAnimationFrame(cursorRafRef.current), []);

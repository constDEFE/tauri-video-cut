...
export const useCursor = ({ canvasRef, duration, isPlaying, isDisabled, setCursor }: Props) => {
	const [hoveredPosition, setHoveredPosition] = useState<number | null>(null);

	const handleMouseMove = useCallback(
		(e: MouseEvent) => {
			if (!canvasRef.current || duration <= 0) return;
			setHoveredPosition(newCursor(e.clientX, canvasRef.current, duration));
		},
		[canvasRef, duration]
	);

	const handleMouseLeave = useCallback(() => setHoveredPosition(null), []);

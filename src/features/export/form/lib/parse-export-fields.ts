...
type ExportParseError = { success: false; error: string };
type ExportParseSuccess = { success: true; data: ExportSettings };

export type ExportParseResult = ExportParseError | ExportParseSuccess;
...
export const parseExportFields = (
	...
	if (!output) {
		return { success: false, error: "Select output folder" };
	}
	...
	if (!isValidPrefix(prefix)) {
		return { success: false, error: "Prefix must be a valid Windows file name" };
	}
